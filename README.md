...
Para probar la narración del ritual en local sin levantar el backend, creá un `.env.local` con:
...
En producción, no cargues `ELEVENLABS_API_KEY` en el proyecto frontend. La app desplegada solo necesita:

```bash
VITE_RITUALES_API_BASE_URL=https://rituales-backend.vercel.app/api
```

La clave de ElevenLabs va como secreto del backend.
